...
# Last modification time: <2019-09-03 12:04:08>
...
import syncopy.connectivity.connectivity_analysis
...
def corr(trl_dat, dimord, removemean=None, pownorm=True, complex="abs",
    ...
    if len(shp) == 4:
        ...
        outdtype = syncopy.connectivity.connectivity_analysis.complexDTypes[complex]
        isSpectral = True
    else:
        (_, nChannel) = shp
    ...
    syncopy.connectivity.connectivity_analysis.complexConversions
...
class ConnectivityCorr(ComputationalRoutine):
    ...
    def process_metadata(self, data, out):
        ...
        out.channel1 = data.channel  # FIXME: this should become `data.channel[chanidx1]` at some point
        out.channel2 = data.channel  # FIXME: this should become `data.channel[chanidx2]` at some point
        if out.__class__.__name__ == "SpectralData":
            out.freq = data.freq
        out.trialinfo = data .trialinfo
